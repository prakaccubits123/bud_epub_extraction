    ...
    for child in elem.children:
        ...
        if isinstance(child, NavigableString):
            ...
            if child.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                ...
                image_parent = child.find_parent("div", class_="image")
                parent_figure = child.find_parent("div", class_="mediaobject")
                ...
                if parent:
                    ...
                    next_sibling = image_parent.find_next_sibling()
                    if next_sibling:
                    ...
                    img_parent = parent_figure.find_parent(
                        "div", class_="figure-contents"
                    )
                    if img_parent:
                        fig_parent = img_parent.find_parent("div", class_="figure")
                        if fig_parent:
                            figcaption = fig_parent.find("p", class_="title")
                            if figcaption:
                                img["caption"] = figcaption.get_text(strip=True)
                                print("this is image_caption", img["caption"])
                ...
                equaiton_contents = child.find("div", class_="equation-contents")
                print("equation image here")
                if equaiton_contents:
                    equation_media = equaiton_contents.find("div", class_="mediaobject")
                    if equation_media:
                        equation_image = equation_media.find("img")
...
for book in publisher_collection.find():
    if (
        "publishers" in book
        and book["publishers"]
        and book["publishers"][0].startswith("No Starch")
    ):
        if "s3_key" in book:
            s3_key = book["s3_key"]
...
# get_book_data("Eloquent JavaScript 2nd Edition (9781457189821)")
