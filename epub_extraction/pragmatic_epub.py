...
def extract_data(elem, book, filename, section_data=[]):
    for child in elem.children:
        ...
        if isinstance(child, NavigableString):
            ...
            # code inside table tag with processedcode class
            elif child.name == "table" and "processedcode" in child.get("class", []):
                print("code here")
                code = ""
                code_id = generate_unique_id()
                code_cells = child.find_all("td", class_="codeline")
                ...
                if prev_sibling:
                    ...
                    if prev_sib_class == "figurecaption":
...
for book in publisher_collection.find():
    if (
        "publishers" in book
        and book["publishers"]
        and book["publishers"][0].startswith("Pragmatic")
    ):
        if "s3_key" in book:
            s3_key = book["s3_key"]
...
# get_book_data("Practical A_B Testing (9798888650257)")
