    ...
    for child in elem.children:
        ...
        if isinstance(child, NavigableString):
            ...
            if child.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                ...
                parent3 = child.find_parent("p", "image")
                # parent4 = child.find_parent("div", "figure")
                ...
                if parent:
                    ...
                    next_sibling = parent3.find_next_sibling()
                    if next_sibling:
                        next_sib_class = next_sibling.get("class", [""])[0]
                        if (
                            next_sib_class == "figcaption"
                            or next_sib_class == "fig-caption"
                        ):
                ...
                if tab_parent:
                    ...
                    if next_sibling:
                        next_sib_class = next_sibling.get("class", [""])[0]
                        if next_sib_class == "table-caption":
                            ...
                            if prev_sibling:
                                ...
                                if prev_sib_class == "tcaption":
                                    ...
                                    print("this is table caption", caption_text)
                                elif prev_sib_class == "tcaption1":
                                    prev_sibling2 = prev_sibling.find_previous_sibling()
                                    if prev_sibling2:
                                        prev_sibling2_class = prev_sibling2.get(
                                            "class", [""]
                                        )[0]
                                        if prev_sibling2_class == "tcaption":
                                            caption_text = prev_sibling2.get_text(
                                                strip=True
                                            )
...
for book in publisher_collection.find():
    if (
        ...
        and book["publishers"]
        and (
            book["publishers"][0].startswith("A K Peters/CRC Press")
            or book["publishers"][0].startswith("CRC Press")
        )
    ):
        if "s3_key" in book:
            s3_key = book["s3_key"]
