...
def parse_html_to_json(html_content, book, filename, db):
    ...
    section_data = extract_data(soup.find("body"), book, filename, db, section_data=[])
    return section_data
    ...
    for child in elem.children:
        ...
        if isinstance(child, NavigableString):
            ...
            # handle table as image p tag with class name timage and image-t
                ...
                cls in child.get("class", []) for cls in ["timage", "image-t", "imaget"]
            ):
                print("hello")
                caption = ""
                next_sibling = child.find_next_sibling()
            ...
            # code inside p tag with class name code1
                ...
                for cls in ["code1", "imagepre", "imageprei"]
            ):
                print("code here")
                code_tag = child.find("code")
...
if __name__ == "__main__":
    ...
    for book in publisher_collection.find():
        if (
            "publishers" in book
            and book["publishers"]
            and book["publishers"][0].startswith("Mc")
        ):
            if "s3_key" in book:
