...
db2 = mongo_init("epub_productivity")
oct_toc = db2.oct_toc
    ...
    for child in elem.children:
        ...
        if isinstance(child, NavigableString):
            ...
            if child.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                ...
                parent = child.find_parent("div", class_="fig")
                parent2 = child.find_parent("figure")
                if parent:
                    figcap = parent.find("p", class_="fig-caption")
                    if figcap:
                        img["caption"] = figcap.get_text(strip=True)
                        print("this is image_caption", img["caption"])
                elif parent2:
                    cap_parent = parent2.find("figcaption")
                    if cap_parent:
                        figcap = cap_parent.find("p")
                        print("yes here")
                        if figcap:
                            img["caption"] = figcap.get_text(strip=True)
                            print("this is image caption", img["caption"])
                        else:
                            img["caption"] = cap_parent.get_text(strip=True)
                            print("this is image caption", img["caption"])
                ...
                parent = child.find_parent("div", class_="table-wrap")
                if tabcap:
                    caption_text = tabcap.get_text(strip=True)
                    print("this is table caption", caption_text)

                elif parent:
                    span = parent.find("span", class_="table-label")
                    span2 = parent.find("span", class_="caption")
                    caption_text += span.get_text(strip=True) if span else ""
                    caption_text += span2.get_text(strip=True) if span2 else ""
                    print("this is table caption", caption_text)
...
for book in publisher_collection.find():
    if (
        "publishers" in book
        and book["publishers"]
        and book["publishers"][0].startswith("Productivity")
    ):
        if "s3_key" in book:
            s3_key = book["s3_key"]
            print(s3_key)
...
print("total not_s3", len(not_s3))

# get_book_data(
#     "Customer Relationship Management (CRM) for Medium and Small Enterprises (9781000532746)"
# )
