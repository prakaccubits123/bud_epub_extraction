        ...
        if isinstance(child, NavigableString):
            ...
            if child.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                ...
                if parent:
                    ...
                    cap_parent = parent2.find_parent("div", class_="fig-heading")
                    if cap_parent:
                    ...
                    fig_parent = child.find_parent("docText")
                    if fig_parent:
                        cap_parent = fig_parent.find_parent("div", class_="center")
                        if cap_parent:
                            figcap = cap_parent.find("h5", class_="docFigureTitle")
                            if figcap:
                                img["caption"] = figcap.get_text(stirp=True)
                if section_data:
                ...
                child.name == "p"
                and child.get("class")
                and child["class"][0].startswith("pre")
            ):
...
for book in publisher_collection.find():
    if (
        "publishers" in book
        and book["publishers"]
        and book["publishers"][0].startswith("Cisco")
    ):
        if "s3_key" in book:
            s3_key = book["s3_key"]
